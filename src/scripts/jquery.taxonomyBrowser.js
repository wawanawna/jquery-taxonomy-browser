  ...
  $.taxonomyBrowser = function(el, options){
    ...
    base.init = function(){
      ...
      if (base.options.source == 'json') {
        if (typeof base.options.json == 'string' || base.options.json instanceof String) {
          $.ajax({
        ...
        else if (jQuery.isArray(base.options.json)) {

          var tax = [];
          var deferreds = [];
          var requestsCount = base.options.json.length;

          for (var i = 0; i < base.options.json.length; i++) {
            deferreds.push(
                    $.ajax({
                      url:base.options.json[i],
                      dataType:'text'
                    }).success(
                            function (taxPart, status, response) {
                              requestsCount--;
                              if (response.status === 200) {
                                tax.push.apply(tax, JSON.parse(taxPart));
                              }
                            }).fail(function (taxPart, status, response) {
                              requestsCount--;
                            }));
          }

          var processTaxonomy = function () {
            if (requestsCount > 0) {
              setTimeout(processTaxonomy, 100);
              return;
            }

            base.processObject(tax);
          };

          processTaxonomy();
        }
        else {
          throw new Error("Base.options.json must be string or array of strings, but has value: " + base.options.json);
        }
    ...
    base.processJSON = function(tax){
      base.processObject(JSON.parse(tax));
    };
     ...
     * @return {[type]}     [description]
     */
    base.processObject = function(taxonomy){
      if (base.options.taxonomyProcessHook) {
        taxonomy = base.options.taxonomyProcessHook(taxonomy)
      }
